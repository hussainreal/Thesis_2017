...
netStatsApp.controller('StatsCtrl', function($scope, $filter, $localStorage, socket, _, toastr) {
	...
	.on('data', function incoming(data) {
		console.info("getting data stat....." + JSON.stringify(data.data));
	...
	{
		console.log("init socket listener fired");
		...
		switch(action)
		{
			...
			case "update":
				...
				if( index >= 0 && !_.isUndefined($scope.nodes[index]) && !_.isUndefined($scope.nodes[index].stats) )
				{
					...
					if ( !_.Undefined(data.stats.contracts)) {
							$scope.costBreakdown = data.stats.contracts;
						}

					$scope.ethToUSD = data.stats.ethToUSD;
			...
			case "stats":
				...
				if( !_.isUndefined(data.id) && index >= 0 )
				{
					...
					if( !_.isUndefined(node) && !_.isUndefined(node.stats) )
					{
						...
						if ( !_.isUndefined(data.stats.contracts)) {
							$scope.costBreakdown = data.stats.contracts;
						}
						$scope.ethToUSD = data.stats.ethToUSD;
